...
class ImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        print(f"[{name}] 原始 A: shape={img.shape}, dtype={img.dtype}, range=[{img.min()}, {img.max()}]")
        print(f"[{name}] 原始 B: shape={img_B.shape}, range=[{img_B.min()}, {img_B.max()}]")

        [img, img_B], _ = self.augm.transform([img, img_B],[], to_tensor=self.to_tensor)
        print(f"[{name}] 增强后 A: shape={img.shape}, range=[{img.min()}, {img.max()}]")
        print(f"[{name}] 增强后 B: shape={img_B.shape}, range=[{img_B.min()}, {img_B.max()}]") 
