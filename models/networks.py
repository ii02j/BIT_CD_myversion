...
class BASE_Transformer(ResNet):
    ...
    def forward(self, x1, x2):
        ...
        x1 = self.forward_single(x1)
        print(f"after forward_single: x1.shape = {x1.shape}")
        x2 = self.forward_single(x2)
        print(f"after forward_single: x2.shape = {x2.shape}")

        #  forward tokenzier
        if self.tokenizer:
            token1 = self._forward_semantic_tokens(x1)
            print(f"after _forward_semantic_tokens: token1.shape = {token1.shape}")
            token2 = self._forward_semantic_tokens(x2)
            print(f"after _forward_semantic_tokens: token2.shape = {token2.shape}")
        else:
        ...
        x = torch.abs(x1 - x2)
        print(f"after torch.abs: x.shape = {x.shape}")
        if not self.if_upsample_2x:
            x = self.upsamplex2(x)
            print(f"after upsamplex2: x.shape = {x.shape}")
        x = self.upsamplex4(x)
        print(f"after upsamplex4: x.shape = {x.shape}")
        # forward small cnn
        x = self.classifier(x)
        print(f"after classifier: x.shape = {x.shape}")
        if self.output_sigmoid:
            x = self.sigmoid(x)
        return x
